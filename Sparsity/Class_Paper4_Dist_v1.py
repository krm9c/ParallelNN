...
#############################################################################################
    ...
    def Lambda_optimizer(self, lr):
        ...
        for (i,weight) in enumerate(self.Lambda_List):
            weight_update = -1*tf.gradients(self.classifier["Overall_cost"], weight)[0]
            # #Generate the updated variables
            train_list.append((weight_update, weight))
...
#############################################################################################
    def Def_Network(self, classes, Layers, act_function, batch_size, gamma, back_range = 1):
        ...
        with tf.name_scope("Trainer_Network"):            
            ...
            # Define the first layer of cost function
            ...
            # The total lambda cost
            self.Print_Cost_grad.append(tf.losses.mean_squared_error(self.classifier['lambda'+str(i)],0*self.classifier['lambda'+str(i)]))
        ...
        with tf.name_scope("Classifier"):
            ...
            # The total lambda cost
            self.Print_Cost_grad.append( tf.losses.mean_squared_error(self.classifier['lambdaclass'],0*self.classifier['lambdaclass']) )
...
################################################################################################################           
    def New_Formulation(self, classes, Layers, act_function, batch_size, gamma, back_range = 1, rho = 0.8):
        ...
        i = 1 
        key = 'FL_layer_1' + str(i)
        self.classifier['Zs' + key] =  tf.matmul(self.Deep['FL_layer_10'], self.classifier['Weight'+key])+self.classifier['Bias'+key]
        ...
        for i in range(2, len(Layers)):
            ...
            tf.matmul( act_function( self.classifier['Zs' + key_prev] ), self.classifier['Weight'+key] ) + self.classifier['Bias'+key] )  
        ...
        # Final layer Cost
        Temp_Cost = rho*tf.losses.mean_squared_error( act_function(self.classifier[key]) , tf.matmul( self.classifier['Zs'+key_prev],\
...
############################################################## Design the trainer for the  network
            ...
            #The overall cost function
                ...
                self.Trainer["Cost_op"]   = tf.norm(self.Print_Cost_grad, ord='euclidean')
                # Distributed Op
