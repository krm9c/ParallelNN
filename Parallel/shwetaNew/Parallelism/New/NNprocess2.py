    ...
    def train(self):
        ...
        for j in range(1,self.steps+1):
            ...
            if self.process_id == self.nr_processes:
                ...
                if j%5 == 0:
                    print "Step:", j, "Total Cost:", self.d[j], "Top Layer Cost", w_cost
                if (j-1)%50 == 0 and not (j-1) == 0:
                    train_acc, cost_func = self.test(self.X_train, self.y_train, j-1)
                    print ("Step:", j-1, "Total Cost:", self.d[j-1], "Acc:", train_acc, "Softmax:", cost_func)
            ...
            # First Layer
                ...
                for m in range(1, 100*self.z_update_steps+1):
                    ...
                    self.model.classifier['z_next']: z_next, \
                    self.model.classifier['z_prev']: z_prev})    
                    #print ("Process:", self.process_id, "Step:", j,temp, "W Self Cost:", w_cost)
...
#                 for temp in range(1,self.w_update_steps+1):
                    ...
                    self.model.classifier['z_next']: z_next, \
                    self.model.classifier['z_prev']: z_prev})    
                    #print ("Process:", self.process_id, "Step:", j,temp, "W Next Cost:", w_cost)
    ...
    def test(self, input_data, target_data, num):
        #print "Testing !!"
        nr_processes = self.nr_processes+1
        #dir_path = '/media/krm9c/My Book/ShwetaExt/Weights_1/'
