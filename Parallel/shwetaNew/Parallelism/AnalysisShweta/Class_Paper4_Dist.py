    ...
    def Dist_optimizer(self, lr):
        ...
        for (i,z) in enumerate(self.Zs):
            if i==0:
                ...
                # Weight and bias updates
                weight_update = tf.gradients(cost,weight1)[0]+ 0.0001*weight1
                bias_update   = tf.gradients(cost,bias1)[0]  + 0.0001*bias1
                train_list_weight.append((weight_update, weight1))
                train_list_weight.append((bias_update, bias1))   

                # Weight and bias updates
                weight_update = tf.gradients(cost,weight2)[0]+ 0.0001*weight2
                bias_update   = tf.gradients(cost,bias2)[0]  + 0.0001*bias2
...
###############################################################Design the trainer for the new network
            with tf.name_scope("Trainers_Dist"): 
                ...
                self.classifier["Total_Z_cost"] = rho*tf.add_n(self.Cost) 
                # The final cost function
