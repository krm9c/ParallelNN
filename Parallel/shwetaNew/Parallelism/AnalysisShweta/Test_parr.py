...
def Analyse_custom_Optimizer_GDR_old(X_train, y_train, X_test, y_test, kappa, batch_size,back_range):
    ...
    T = sample_Z(X_test, X_test.shape[0], X_test.shape[1], kappa=0)
    ...
    try:
        ...
        for i in tqdm(t):
            ...
            for batch in iterate_minibatches(X_train, y_train, Train_batch_size, shuffle=True):
                ...
                #     print("Overall Cost", cost_Total[i], "Learn", cost_M1[i], "Z Cost", cost_M3[i])
                #     print("---------------------------------------------------------------------------------------------------------")
...
filename = 'paper_4_Noise_Training_Gau_relu_5fdsfg.csv'
