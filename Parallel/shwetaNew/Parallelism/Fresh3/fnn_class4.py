...
class learners():
    ...
    def first_cost_function(self, activation, optimizer):
        ...
        z_next1 = act(tf.matmul(self.classifier['z_next1'], self.classifier['w_next1'] ) + self.classifier['b_next1'] )
        z_next2 = act(tf.matmul(self.classifier['z_next2'], self.classifier['w_next2'] ) + self.classifier['b_next2'] ) 
        zFinal  = tf.add(tf.matmul( self.classifier['z_self'], self.classifier['w_lay_next']),self.classifier['b_lay_next'])
        
        
        self.classifier['Local_Cost_H']= self.classifier['lambda_value']*tf.nn.l2_loss(tf.subtract(self.classifier['z_next2'], z_next1) ) \
        ...
        z_update = tf.gradients(self.classifier['Local_Cost_H'], self.classifier['z_next2'])[0]
        self.classifier['train_list_Z'].append((z_update, self.classifier['z_next2']))
    ...
    def init_NN_custom(self, pr_id, nr_processes, classes, depth, batch_size, activation='tanh', optimizer='Adam'):
        with tf.name_scope("Trainers"):  
            ...
            if pr_id == 1:
                ...
                #self.classifier['z_next2'] = tf.nn.tanh(tf.matmul(self.classifier['z_next1'], self.classifier['w_next1'] ) + self.classifier['b_next1'] )
