...
dir_path = '/media/krm9c/My Book/ShwetaExt/Weights_11/'
...
class NNProcess(Process):
    ...
    def init_nets(self):
        ...
        if self.process_id == 1:
            ...
            weight = self.model.sess.run(self.model.classifier['w_next1'])
            bias = self.model.sess.run(self.model.classifier['b_next1'])
            Weights.append(weight)
            Weights.append(bias)
            write_file(w_next_file_dir, w_next_file_path, Weights, self.process_id)

            w_next2_file_dir = dir_path+'Layer_'+str(self.process_id+2)+'/'
            Weights = []
            w_next2_file_path  = 'w_'+str(self.process_id+2)+'_'+'0'
            weight = self.model.sess.run(self.model.classifier['w_next2'])
            bias = self.model.sess.run(self.model.classifier['b_next2'])
            Weights.append(weight)
            Weights.append(bias)
            write_file(w_next2_file_dir, w_next2_file_path, Weights, self.process_id)
            ...
            Weights = []
            w_self_file_path  = 'w_'+str(self.process_id+2)+'_'+'0'
            weight = self.model.sess.run(self.model.classifier['w_self'])
            bias = self.model.sess.run(self.model.classifier['b_self'])
            Weights.append(weight)
            Weights.append(bias)
    ...
    def train(self):
        ...
        for j in range(1,self.steps+1):
            ...
            if self.process_id == 1:
                ...
                read_w_path_last  = dir_path+'Layer_'+str(self.process_id+3)+'/w_'+str(self.process_id+3)+'_'+str(j-1)
                ...
                # T
                read_t_path_next  = dir_path+'Layer_'+str(self.nr_processes+2)+'/t_'+str(self.nr_processes+2)+'_'+str(j)
                t_next = read_file(read_t_path_next, self.process_id)
                if 'NoneType' in str(type(t_next)):
                    while 'NoneType' in str(type(t_next)):
                        t_next = read_file(read_t_path_next, self.process_id)
                # print(t_next.shape, w_lay_n.shape, b_lay_n.shape)
                ...
                w_next2_file_dir = dir_path+'Layer_'+str(self.process_id+2)+'/'
                Weights = []
                w_next2_file_path  = 'w_'+str(self.process_id+2)+'_'+str(j)
                weight = self.model.sess.run(self.model.classifier['w_next2'])
                bias = self.model.sess.run(self.model.classifier['b_next2'])
                Weights.append(weight)
                Weights.append(bias)
    ...
    def test(self, input_data, target_data, num):
        #print "Testing !!"
        Layers_num = self.nr_processes+2
