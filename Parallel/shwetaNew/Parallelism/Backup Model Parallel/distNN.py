...
def get_batches(process_id, nr_processes, steps, X_train, y_train, batch_size):
    ...
    print("Process " + str(nr_processes+1) + " starting...")
    for j in range(1,steps+1):
        ...
        write_file(x_file_dir, x_file_path, x_batch)
        #print ("Process:", nr_processes+1, "Step:", j)
        y_file_dir = dir_path+'Layer_'+str(nr_processes+1)+'/'
        y_file_path  = 't_'+str(nr_processes+1)+'_'+str(j)
        write_file(y_file_dir, y_file_path, y_batch)
    
    print("Process " + str(process_id) + " finished.")
    print("Process " + str(nr_processes+1) + " finished.")
...
def main():
    ...
    for i in range(1, nr_processes+1):
        nn_process = NNProcess(nr_processes, i, depth, classes, lr_z, \
            lr_w, lambda_value, op, batch_size, steps, z_update_steps, d)
