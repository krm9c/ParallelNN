...
class learners():
    ...
    def compute_cost_function(self, lambda_value, act): 
        first_term = lambda_value*(tf.nn.l2_loss(tf.subtract(self.classifier['z_next'], \
    ...
    def testing(self, input_data):
        ...
        self.classifier['z_self_test'] = tf.nn.tanh(tf.add(tf.matmul(input_data, self.classifier['w_self']),self.classifier['b_self']))
    ...
    def last_testing(self, input_data, target_data):
        ...
        self.classifier['z_self_test'] = tf.nn.softmax(tf.add(tf.matmul(input_data, self.classifier['w_self']),self.classifier['b_self']))
    ...
    def init_NN_custom(self, pr_id, nr_processes, classes, Layers, depth, batch_size, lambda_value, act_function=tf.nn.relu,
            ...
            with tf.name_scope("Trainers"): 
                ...
                self.Trainer['Weight_op'], self.classifier['Cost'], \
                    self.classifier['w update'], self.classifier['b update'] = self.W_Custom_Optimizer(self.classifier["learning_rate"], optimizer)
