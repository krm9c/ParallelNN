...
#################################################################################################
    ...
    def last_cost_function(self, activation, optimizer, n_layers):
        ...
        zFinal = act(tf.matmul(self.classifier['z3_hat'], self.classifier['w4'] ) + self.classifier['b4'] )
    ...
    def first_cost_function(self, activation, optimizer, n_layers):
        ...
        opt = get_optimizer_name(optimizer)
        zFinal = act(tf.matmul(self.classifier['z3_hat'], self.classifier['w4'] ) + self.classifier['b4'] )
        self.classifier['Local_Cost_H'] =  self.classifier['lambda_value']*tf.nn.l2_loss(tf.subtract(self.classifier['z3_hat'], self.classifier['z3'])) \
        + tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = zFinal, labels=self.classifier['t_next'], name='Error_Cost'))
    ...
    def init_NN_custom(self, pr_id, nr_processes, classes, depth, batch_size, activation='tanh', optimizer='Adam'):
        with tf.name_scope("Trainers"):  
            ...
            if pr_id == 1:
                ...
                # self.classifier['z4_hat'] = z_variable([batch_size, depth[4]], trainable=False, name='z3_hat')
                # self.Weights.append((self.classifier['w4'], self.classifier['b4']))
                n_layers = 2
                # ## The W, b definitions
                # self.classifier['w5'] = weight_variable([depth[5], depth[6]], trainable=False, name='w_self')
                # self.classifier['b5'] = bias_variable([depth[6]], trainable=False, name='b_self')
                self.Weights.append((self.classifier['w4'], self.classifier['b4']))
                # The cost and variables
                self.classifier['cost'+str(pr_id)], self.Trainer['Weight_op'] = self.cost_function(pr_id, nr_processes, activation, optimizer, n_layers)
