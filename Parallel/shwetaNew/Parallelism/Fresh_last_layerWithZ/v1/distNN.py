...
def get_batches(process_id, nr_processes, steps, X_train, y_train, batch_size):
    ...
    for j in range(1,steps+1):
        ...
        arr = random.sample(range(0, X_train.shape[0]), batch_size)
        for idx in arr:
        ...
        x_file_dir = dir_path+'Layer_'+str(0)+'/'
        x_file_path  = 'z_'+str(0)+'_'+str(j)
        write_file(x_file_dir, x_file_path, x_batch, 0)
        
        
        #print ("Process:", nr_processes+2, "Step:", j)
        y_file_dir = dir_path+'Layer_'+str(4)+'/'
        y_file_path  = 't_'+str(4)+'_'+str(j)
        write_file(y_file_dir, y_file_path, y_batch, 4)



    print("Process " + str(process_id) + " finished.")
    print("Process " + str(4) + " finished.")
...
def main():
    ...
    #Parameters
    X_train, y_train, X_test, y_test = load_data('rolling', dataset_path)
    X_train = preprocessing.scale(X_train)
    X_test = preprocessing.scale(X_test)
    arr = random.sample(range(0, X_train.shape[0]), 1000)
    X_train = X_train[arr,:]
    y_train = y_train[arr,:]
    arr = random.sample(range(0, X_test.shape[0]), 1000)
    X_test = X_test[arr,:]
    y_test = y_test[arr,:]
    print("X Train shape", X_train.shape, X_test.shape)
    depth = [11, 100, 100, 100, 100, 4]
    classes = 4
    lr_z = 0.1
    lr_w = 0.0001
    lambda_value = 1.1
    ...
    for i in range(1, nr_processes+1):
        ...
        nn_process = NNProcess(nr_processes, i, depth, classes, lr_z, \
            lr_w, lambda_value, activation, optimizer, batch_size, steps, z_update_steps, w_update_steps, d)
        nn_process.set_train_val(X_train, y_train, X_test, y_test)
        processes.append(nn_process)

    print "Length of processes is",len(processes)
