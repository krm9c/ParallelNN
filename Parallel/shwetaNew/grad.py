...
if paras['num_NNlayers'] == 1:
    ...
    ''' W = tf.Variable(np.random.rand(paras['num_features'], paras['num_labels']).astype(np.float32), name='Weight_Layer_0')
    b = tf.Variable(np.random.rand(paras['num_labels']).astype(np.float32), name='Bias_Layer_0') '''
    ...
    ''' Wi = tf.Variable(np.random.rand(paras['num_features'], paras['hidden_nodes']).astype(np.float32), name='Weight_Layer_0')
    bi = tf.Variable(np.random.rand(paras['hidden_nodes']).astype(np.float32), name='Bias_Layer_0')
    Wo = tf.Variable(np.random.rand(paras['hidden_nodes'], paras['num_labels']).astype(np.float32), name='Weight_Layer_' + str(paras['num_NNlayers']-1))
    bo = tf.Variable(np.random.rand(paras['num_labels']).astype(np.float32), name='Bias_Layer_'+str(paras['num_NNlayers']-1)) '''
    ...
    for i in range(paras['num_NNlayers']-2):
        ...
        ''' W2 = tf.Variable(np.random.rand(paras['hidden_nodes'], paras['hidden_nodes']).astype(np.float32), name='Weight_Layer_' + str(i+1))
        b2 = tf.Variable(np.random.rand(paras['hidden_nodes']).astype(np.float32), name='Bias_Layer_' + str(i+1)) '''
...
WB = []
WB.extend(Hidden_Weights)
WB.extend(Hidden_Bias)


# Use gradient descent to calculate the gradients with respect to our parameters.
# This is how we'll minimize the loss.
opt = tf.train.GradientDescentOptimizer(paras['learning_rate'])
train_step = opt.minimize(cross_entropy)
grads_and_vars = [tf.gradients(cross_entropy, element) for element in WB]
...
for j in range(paras['steps']):
    ...
    grad_vals = sess.run([grad for grad in grads_and_vars], feed_dict={x: x_batch, y_: y_batch})

    ''' train_step.run(feed_dict={x: X_train, y_: y_train})
    grad_vals = sess.run([grad for grad in grads_and_vars], feed_dict={x: X_train, y_: y_train}) '''
    
    # Add the gradients from each step to our dictionary
    for grad_val, var in zip(grad_vals, WB):
    ...
    if j%100 == 0:
        print "Step", j
        #print("Loss", cross_entropy.eval(feed_dict={x: X_train, y_: y_train}))
...
print("Final Test Accuracy", accuracy.eval(feed_dict={x: X_test, y_: y_test})*100)

#print var_to_grad
#print len(var_to_grad)

fig = plt.figure()
for l in range(paras['num_NNlayers']):
    gradients = var_to_grad['Weight_Layer_' +str(l)+':0']
