...
def perturbData(X, m, n):
    #return (X+np.random.normal(0, 0.1, size=[m, n]))
    return (X+(10*np.random.uniform(-0.1,0.1,size=[m,n])))
...
for j in range(steps):
    ...
    for k in xrange(100):
        ...
        feed_dict={model.Deep['Conv_Layer_00']: x_batch_perturbed, model.classifier['Target']: \
